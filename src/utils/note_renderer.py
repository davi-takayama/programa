...
class NoteRenderer:
    ...
    def eighth(
        ...
        has_sharp: List[Literal["none", "sharp", "flat", "natural"]] = ["none"],
        ...
        if len(has_sharp) != len(positions) and len(has_sharp) == 1:
        ...
        def draw_note(note_pos, note_has_sharp, note_color):
            if index == len(positions) - 1 and len(positions) % 2 != 0:
                return self.single_eighth(
                    note_pos[0], note_pos[1], note_has_sharp, note_color
                )
            else:
                return self.quarter(
                    note_pos[0], note_pos[1], note_has_sharp, note_color
